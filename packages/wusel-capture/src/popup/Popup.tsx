...
const MODES: ModeDef[] = [
	{
		mode: 'fullpage',
		label: 'Full page',
		description: 'The entire page, including below the fold',
...
export function Popup() {
	...
	async function startCapture(mode: CaptureMode) {
		...
		try {
			...
			const request: CaptureRequest = { type: 'CAPTURE', mode, tabId: tab.id, windowId: tab.windowId };
		...
		<div className="flex w-80 flex-col gap-3 p-4">
			<header className="flex items-center gap-2">
				...
				<div className="leading-tight">
					...
					<p className="text-xs text-muted-foreground">Screenshot, annotate, send to Claude Code</p>
				</div>
			</header>
			...
			{error ? (
				...
				<p className="px-1 text-xs text-muted-foreground">
					Does not work on internal pages (chrome://, Web Store).
				</p>
			)}
		</div>
	);
